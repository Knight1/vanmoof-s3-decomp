...
| `0x40022090` | 32 b | write | `bim_panic_indicate`, `bim_flash_prepare`, `FUN_00057138` | `GPIO_BASE (0x40022000) + DOUTSET31_0 (0x90)` — set-only alias for `DOUT31_0`. Used by three callers with three different DIO bits: `1<<2` (DIO2, panic LED), `1<<3` (DIO3, flash-busy), `1<<4` (DIO4, flash-op-in-progress). |
| `0x400220A0` | 32 b | write | (pending: `FUN_00057188` and other call sites) | `GPIO_BASE + DOUTCLR31_0 (0xA0)` — set-only alias for clearing DIO bits. Used as a literal-pool-shared constant: many call sites load `0x400220A0` and either use it directly (`FUN_00057188` writes `1<<4` to clear DIO4) or `subs r0, #16` to convert it to `DOUTSET31_0` at `0x40022090`. |
