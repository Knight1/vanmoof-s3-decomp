...
/* UART5 ("bus") byte transport — the second inter-module serial link, carrying
...
/* Locked single-byte TX into the UART5 TX ring; implicitly returns
 * ringbuf_push_byte's status (1 = pushed, 0 = ring full). OEM 0x0803639C. */
int bus_tx_enqueue_byte(uint8_t b);

/* Locked single-byte RX from the UART5 RX ring into *out; implicitly returns
 * ringbuf_get_byte's status (1 = byte produced, 0 = ring empty). OEM 0x080363EC. */
int bus_rx_byte_locked(uint8_t *out);

/* UART5 RX/TX byte-pump interrupt service routine (OEM 0x08036424), invoked via a
