...
| 0   | in-progress |
| 519 | decomp-c — `vectors.c` (25), `tim.c` (9), `stc3115.c` (16), `console_edit.c` (12), `systick.c` (3), `console.c` (65), `scheduler.c` (8), `exceptions.c` (10), `panic.c` (2), `app.c` (25), `util.c` (6), `system_stm32f413.c` (1), `ssp.c` (22), `motor.c` (9), `flash.c` (9), `crc.c` (6), `audio.c` (1), `log.c` (10), `sensor.c` (9), `uart.c` (8), `bus.c` (3), `net.c` (2), `gpio.c` (5), `eeprom.c` (3), `i2c.c` (21), `watchdog.c` (6), `states.c` (7), `modem.c` (56), `ble.c` (5), `ble_read.c` (3), `rtc.c` (9), `update.c` (1), `main.c` (4), `battery.c` (29), `display.c` (33), `lighting.c` (9), `display_requests.c` (data: 38 descriptors), `shifter.c` (39), `lis3dh.c` (26), `comm.c` (2) — see per-module log below |
...
- **`flash_read_config_with_crc_restore` SOURCED + SMS remote-control documented + 9 more validated — all PASS.**
  ...
  `config_persist_dual_bank`: copy bank A `0x08008000` → CRC-verify 0x34 words → on fail log
  `"  ERR:Read flash copy"` + read bank B `0x0800C000` → if B valid re-commit BOTH banks to heal A; returns
  0 if either valid / 1 if both corrupt; the 0xC0-byte payload at `out+0x10` passed by value, matching the
  OEM stack copy). **`sim_iccid_check` re-validated PASS** — the prior inner-deref fix (`*(char**)(ctx+0x3E8)+0x50`)
  is confirmed correct against the OEM `ldr [ctx,#0x3E8]; +0x50`. **`modem_sms_dispatch_command` documented in
  `docs/modem.md`** (the inbound-SMS remote-control surface): the `#<8-char code>*<cmd>` wire format, all 5
  commands (key/TrackingOn/TrackingOff/ping/makeNoise), the `ping` JSON status payload + a `curl` example, and
  the **8-char-code security analysis** — it's a backend-supplied **correlation token, NOT an auth secret**:
  the firmware never stores or validates it (nor the sender number); the only anti-theft auth is the SIM lock
  (`sim_iccid_check`) + the bike's private provisioned number. Two doc/comment corrections folded in: the
  TrackingOn gate is **GPIOC PC2** (not GPIOB), and the ICCID is `*(ctx+0x3E8)+0x50` (inner deref). **8 more
  validated PASS** (modem SMS-read/location-send/ctx-activate steps, console distance/shipping,
  battery_on_detect_step, shifter_seq_status_poll_step, ssp_ble_dispatch) — 0 bugs. flash.c decomp-c 8→9
  (total 518→519); validation running-total ~96 functions across 8 batches, 11 real bugs fixed.
