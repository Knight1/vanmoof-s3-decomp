/*
 * bus.c — VanMoof S3 mainware UART5 ("bus") byte transport.
 *
 * UART5 is the second inter-module serial link, carrying the Modbus-RTU traffic
 ...
 * moves bytes between the UART5 register block and the software ring buffers.
 *
 * Memory model (resolved from the OEM literal pool):
 *   0x20009964  RAM slot holding the UART5 register-block pointer (set by
 *               uart5_init); the dev block is reached by one extra dereference.
 *   0x20001A44  the shared serial context; UART5's TX ring handle is at +0x330
...
/* UART5 register-block handle (pointer-to-pointer) and the serial context base
 * holding its ring handles. */
#define UART5_HANDLE   (*(volatile uint32_t * volatile *)0x20009964u)
#define UART5_CTX      0x20001A44u
#define UART5_TX_RING  (*(ringbuf_t * volatile *)(UART5_CTX + 0x330u))
#define UART5_RX_RING  (*(ringbuf_t * volatile *)(UART5_CTX + 0x334u))

/* Locked single-byte TX into the UART5 TX ring (OEM bus_tx_enqueue_byte,
 * 0x0803639C). */
int bus_tx_enqueue_byte(uint8_t b)
{
    volatile uint32_t *dev = UART5_HANDLE;

    dev[0xC / 4] &= ~0x80u;                        /* mask TXEIE */
    __asm volatile ("dsb 0xf" ::: "memory");
    __asm volatile ("isb 0xf" ::: "memory");

    uint32_t rc = ringbuf_push_byte(UART5_TX_RING, b);

    dev = UART5_HANDLE;                            /* OEM re-loads the handle */
    dev[0xC / 4] |= 0x80u;                         /* re-enable TXEIE */
    return (int)rc;
}

/* Locked single-byte RX from the UART5 RX ring (OEM bus_rx_byte_locked,
 * 0x080363EC). */
int bus_rx_byte_locked(uint8_t *out)
{
    ...
    uint32_t rc = ringbuf_get_byte(UART5_RX_RING, out);
...
/* UART5 RX/TX byte-pump ISR (OEM 0x08036424), invoked via a thin vector
...
void uart5_irq_handler(void)
{
    ...
    if ((sr & 0xFu) == 0 && (sr & 0x20u) != 0 && (cr1 & 0x20u) != 0) {
        ringbuf_push_byte(UART5_RX_RING, (uint8_t)dev[1]);
    }

    volatile uint32_t *d = UART5_HANDLE;
    ...
    if ((sr & 0x80u) != 0 && (cr1 & 0x80u) != 0) {
        uint8_t b;
        if (ringbuf_get_byte(UART5_TX_RING, &b) == 0) {
            UART5_HANDLE[0xC / 4] &= ~0x80u;
        } else {
            UART5_HANDLE[1] = b;
        }
    }
