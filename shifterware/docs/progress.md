...
- **flash_store** (`src/flash_store.c`): `flash_unlock` (12 B), `flash_lock` (14 B), `flash_clear_status` (6 B), `flash_erase_page` (32 B), `flash_erase_pages` (32 B), `flash_get_status` (54 B), `flash_busy_step` (26 B), `flash_wait_status` (44 B), `flash_do_page_erase` (72 B), `flash_program_halfword` (66 B, OEM @ 0x080049B2 — replaced the prior speculative shim; returns FLASH_ST_* status), `flash_program_range` (54 B, OEM @ 0x08003898 — bulk halfword programmer used by the cmd 0x82 OTA chunk path; unlock → clear PGERR/WRPRTERR/EOP → loop `flash_program_halfword` → clear EOP → lock), `settings_set_halfword` (110 B, OEM @ 0x08003178 — RMW one halfword in `FLASH_SETTINGS_PAGE` at `0x08007800`; full 8-halfword read-erase-write per call), `flash_settings_commit` (118 B, OEM @ 0x080031E6 — persist G_STATE_FC + G_COUNTER (BE32) + G_5C_REGS[0..2] across 8 halfwords; clears G_5C_BUSY; called by cmd 0x5C long-form and by main's idle-reset/deferred-commit paths), `settings_read_halfword` (10 B, OEM @ 0x080040A8 — one-line halfword fetch from settings page), `settings_load` (126 B, OEM @ 0x080040B2 — inverse of `flash_settings_commit`; restores G_STATE_FC + G_COUNTER + G_5C_REGS at boot, treats halfword 0 == 0xFFFF as "blank flash"; called once from main's boot prologue).
...
| `0x08003852` |   70 | `cmd_82_emit_ack` | modbus_dispatch | decomp-c | per-chunk OTA ACK: echo G_LONG_BUF[0..5] + CRC, send 8 B |
| `0x08003898` |   54 | `flash_program_range` | flash_store | decomp-c | bulk halfword programmer used by the cmd 0x82 OTA path |
| `0x080038ce` |  100 | `ota_stage_chunk` | modbus_dispatch | decomp-c | pack byte stream into halfwords at G_OTA_STAGE_HW; one-shot flash_program_range |
| `0x0800399e` |   72 | `ota_commit_chunk` | modbus_dispatch | decomp-c | flush staged chunk to flash; on G_OTA_FLUSH_FLAG, also clear all OTA state |
| `0x080039e6` |  160 | `cmd_82_fw_page` | modbus_dispatch | decomp-c | case 0x82 OTA payload entry; decodes total size + walks packets via G_OTA_HEADER_BUF |
...
| `0x0800462a` |   66 | `unused_system_init` | (stdlib) | deferred | dead — HSI enable + CFGR reset + CIR flag-clear preamble |
